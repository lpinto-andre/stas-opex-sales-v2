...
export async function dropAllTables() { const adb = await getDb(); const conn = await adb.connect(); await conn.query('DROP TABLE IF EXISTS pdr_enriched'); await conn.query('DROP TABLE IF EXISTS order_line_first'); await conn.query('DROP TABLE IF EXISTS pdr_clean'); await conn.query('DROP TABLE IF EXISTS pdr_raw'); await conn.close(); }
export async function queryRows<T extends Record<string, unknown>>(sql: string): Promise<T[]> { const adb = await getDb(); const conn = await adb.connect(); const result = await conn.query(sql); await conn.close(); return result.toArray().map((row) => row.toJSON() as T); }
