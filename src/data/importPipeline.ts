...
const phaseProgress: Record<Exclude<ImportPhase, 'idle'>, number> = {
  reading: 0.1,
  parsing: 0.3,
  cleaning: 0.55,
  duckdb: 0.78,
...
export async function importDatasetFile(params: {
  file: File;
  selectedSheet?: string;
  ...
  const mark = (phase: ImportPhase, message: string) => {
  ...
  const doneMark = (phase: ImportPhase, t0?: number) => {
    if (!t0) return;
    console.debug(`[import] ${phase} completed in ${(performance.now() - t0).toFixed(1)}ms`);
  };
  ...
  try {
    ...
    let droppedAmountRows = 0;
    let missingCostRows = 0;
    const customers = new Set<string>();
    const parts = new Set<string>();
    const dates: string[] = [];

    const cleanedRows: CleanRow[] = rawRows
      .map((row) => {
        ...
        const cleanRow: CleanRow = {
          cust_id: String(row[colMap.cust_id] ?? '').trim(),
          cust_name: String(row[colMap.cust_name] ?? '').trim(),
          country: String(row[colMap.country] ?? '').trim(),
          territory: String(row[colMap.territory] ?? '').trim(),
          prod_group: String(row[colMap.prod_group] ?? '').trim(),
          prod_group_desc: String(row[colMap.prod_group_desc] ?? '').trim(),
          part_num: String(row[colMap.part_num] ?? '').trim(),
          line_desc: String(row[colMap.line_desc] ?? '').trim(),
          class_id: String(row[colMap.class_id] ?? '').trim(),
          class_desc: String(row[colMap.class_desc] ?? '').trim(),
          invoice_num: String(row[colMap.invoice_num] ?? '').trim(),
          invoice_date: invoiceIso,
          order_num: String(row[colMap.order_num] ?? '').trim(),
          amount,
          cost: costRaw
        };
        customers.add(cleanRow.cust_id);
        parts.add(cleanRow.part_num);
        return cleanRow;
      })
      .filter((x): x is CleanRow => !!x);

    if (!cleanedRows.length) {
      throw new Error('No valid rows found after cleaning. Check amount and invoice date fields.');
    }
    ...
    await pauseForUI();
    await buildModel(cleanedRows as unknown as Record<string, unknown>[]);
    doneMark('duckdb', duckT);

    checkAbort(params.signal);
    const cacheT = mark('caching', 'Saving to local cache...');
    await pauseForUI();
    const payload = new TextEncoder().encode(JSON.stringify(cleanedRows));
    ...
    await saveDatasetPackage(payload, summary);
    ...
    params.onStatus({ phase: 'error', progress: 1, message: msg, startedAt });
