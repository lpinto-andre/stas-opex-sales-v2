...
export function ExplorerPage() {
  ...
  const selectedTopParts = useMemo(() => topItemsSelection.partNums.slice(0, Math.max(1, topItemsN)), [topItemsSelection.partNums, topItemsN]);
  ...
  useEffect(() => {
    Promise.all([
      ...
      getRevenueByMonthForParts(filters, selectedTopParts), getOrdersByMonthForParts(filters, selectedTopParts), getRevenueTotalsForParts(filters, selectedTopParts), getOrderTotalsForParts(filters, selectedTopParts)
    ]).then(([kpi, rm, om, rf, ofy, rg, og, details, trm, tom, trt, tot]) => {
      ...
      setTopRevByMonth(trm as Record<string, unknown>[]); setTopOrdByMonth(tom as Record<string, unknown>[]); setTopRevTotals(trt as Record<string, unknown>[]); setTopOrdTotals(tot as Record<string, unknown>[]);
  ...
  return <div>
    ...
    <section className="card p-3 mb-3">
      ...
      <p className="text-xs text-[var(--text-muted)] mb-2">Top-items charts are driven by the model selections from the Top Items page.</p>
      ...
      <div className="grid md:grid-cols-5 gap-2 mt-3">
        ...
        <label className="text-xs text-[var(--text-muted)]">Top Items for model charts<input type="number" min={1} value={topItemsN} onChange={(e) => setTopItemsN(Number(e.target.value || 20))} className="card w-full px-2 py-1 mt-1" /></label>
      </div>
    ...
    <div className="grid md:grid-cols-2 gap-4 mb-4">
      ...
      <section className="card p-3 h-72"><h3 className="font-semibold mb-2">Top Items: Revenue by Time</h3><ResponsiveContainer><LineChart data={chartTopRevMonth}><XAxis dataKey="month"/><YAxis/><Tooltip formatter={(v) => currency(Number(v))} contentStyle={tooltipStyle} labelStyle={tooltipLabelStyle} /><Line type="monotone" dataKey="revenue" stroke="#06b6d4"/></LineChart></ResponsiveContainer></section>
      <section className="card p-3 h-72"><h3 className="font-semibold mb-2">Top Items: Orders by Time</h3><ResponsiveContainer><LineChart data={chartTopOrdMonth}><XAxis dataKey="month"/><YAxis/><Tooltip contentStyle={tooltipStyle} labelStyle={tooltipLabelStyle} /><Line type="monotone" dataKey="orders" stroke="#84cc16"/></LineChart></ResponsiveContainer></section>
      <section className="card p-3 h-72"><h3 className="font-semibold mb-2">Top Items: Total Revenue</h3><ResponsiveContainer><BarChart data={chartTopRevTotals}><XAxis dataKey="part_num"/><YAxis/><Tooltip formatter={(v) => currency(Number(v))} contentStyle={tooltipStyle} labelStyle={tooltipLabelStyle} /><Bar dataKey="revenue" fill="#0ea5e9"/></BarChart></ResponsiveContainer></section>
      <section className="card p-3 h-72"><h3 className="font-semibold mb-2">Top Items: Total Orders</h3><ResponsiveContainer><BarChart data={chartTopOrdTotals}><XAxis dataKey="part_num"/><YAxis/><Tooltip contentStyle={tooltipStyle} labelStyle={tooltipLabelStyle} /><Bar dataKey="orders" fill="#65a30d"/></BarChart></ResponsiveContainer></section>
    </div>
