...
import { getOrderTotalsForParts, getOrdersByFYAndPartForParts, getOrdersByFYForParts, getPartYearMetrics, getPartsOrdersByFY, getPartsPriorityRows, getPartsRevenueByFY, getRevenueByFYAndPartForParts, getRevenueByFYForParts, getRevenueCostProfitOverTime, getRevenueTotalsForParts, type Filters } from '@/data/queries';
...
export function TopItemsPage() {
  ...
  const currentSavedView = useMemo<TopItemsSavedView>(() => ({
    ...
    m,
    topN,
    graphicsTopN: Math.max(1, Math.min(10, graphicsTopN || 5)),
    periodMode,
    fromMonth,
    toMonth,
    searchText,
    ...
    selectedParts,
    selectedProdGroups
  }), [k, m, topN, graphicsTopN, periodMode, fromMonth, toMonth, searchText, selectedCustomers, selectedCountries, selectedParts, selectedProdGroups]);
  const {
    activeViewName,
  ...
  const applySavedView = (view: TopItemsSavedView) => {
    ...
    setM(view.m);
    setTopN(view.topN);
    setGraphicsTopN(view.graphicsTopN);
  ...
  useEffect(() => {
    setPageState('top-items', { topN, graphicsTopN: boundedGraphicsTopN, graphicsCollapsed, tableMode, topFilters, k, m, periodMode, fromMonth, toMonth, searchText, selectedCustomers, selectedCountries, selectedParts, selectedProdGroups, weights });
  }, [topN, boundedGraphicsTopN, graphicsCollapsed, tableMode, topFilters, k, m, periodMode, fromMonth, toMonth, searchText, selectedCustomers, selectedCountries, selectedParts, selectedProdGroups, weights, setPageState]);
      ...
      <table className="w-full table-auto text-sm">
        ...
        <tbody>
          {rows.map((row, i) => <tr key={`${row.part_num}-${i}`} className="border-b border-[var(--border)] hover:bg-[var(--surface)]/60 align-top">
            ...
            <td className="px-3 py-2 whitespace-nowrap">{formatFixed(Number(row.revenue_score), 3)}</td><td className="px-3 py-2 whitespace-nowrap">{formatFixed(Number(row.orders_score), 3)}</td><td className="px-3 py-2 whitespace-nowrap">{formatFixed(Number(row.profit_score), 3)}</td><td className="px-3 py-2 whitespace-nowrap">{formatFixed(Number(row.margin_score), 3)}</td><td className="px-3 py-2 whitespace-nowrap">{formatFixed(Number(row.trend_score), 3)}</td><td className="px-3 py-2 whitespace-nowrap">{formatFixed(Number(row.active_score), 3)}</td><td className="px-3 py-2 whitespace-nowrap bg-amber-500/10 font-semibold">{formatFixed(Number(row.final_score), 3)}</td>
            {tableMode === 'complete' && <>
