...
const SYSTEM_PROMPT = [
  ...
  'When context.scope.page is top-items, use only context.signals.topItems and context.filters for ranking claims unless the user explicitly asks for cross-page comparison.',
  'When context.scope.page is top-items, treat topRows as the visible winners and nextRows as the comparison band immediately below the cutoff. Use cutoff when present.',
  'When context.scope.page is top-items, explain why the visible top items surface by referencing the active filters, weights, topRows, nextRows, and cutoff. Focus on business interpretation of the ranking drivers, not a mechanical defense of the formula.',
  'When context.scope.page is top-items, separate facts from interpretation: identify main drivers, shared patterns, outliers, and why the visible rows outrank the next candidates before giving recommendations.',
  'When context.scope.page is top-items, never mention internal field names such as topRows, nextRows, or cutoff in the final answer. Translate them into user-facing language.',
